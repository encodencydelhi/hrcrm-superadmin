...
function Toggle({ checked, disabled, onChange }: { checked: boolean; disabled?: boolean; onChange: () => void }) {
    return (
        <button
            ...
            disabled={disabled}
            onClick={onChange}
            className={`relative inline-flex h-5 w-9 shrink-0 items-center rounded-full transition-colors ${disabled ? 'bg-zinc-200 cursor-not-allowed' : checked ? 'bg-emerald-500' : 'bg-zinc-300'
                }`}
        >
            <span
                className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white shadow transition-transform ${checked ? 'translate-x-4.5' : 'translate-x-1'
                    }`}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-2 mt-1">
                ...
                <div className="lg:col-span-9 flex flex-col gap-2">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-x-4 gap-y-3">
                            {modules.map((mod) => {
                                ...
                                return (
                                    <div
                                        key={mod.key}
                                        className={`flex items-start justify-between gap-2 p-2 rounded-md border ${mod.available ? 'border-zinc-100' : 'border-zinc-100 opacity-60'
                                            }`}
                                    >
                    ...
                    <div className="flex items-center justify-between mt-1">
                        <button onClick={() => router.push(`/super-admin/import-employees`)} className="flex items-center gap-1.5 h-9 px-3 bg-white border border-zinc-200 rounded-md text-[12px] font-semibold hover:bg-zinc-50 transition-colors shadow-sm text-zinc-700">
