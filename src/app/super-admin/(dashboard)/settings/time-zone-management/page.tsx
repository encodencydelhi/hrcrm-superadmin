...
export default function TimeZoneManagementPage() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await api.get('/super-admin/time-zones');
  ...
  const handleEdit = (item: TimeZone) => {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Are you sure you want to delete this time zone?')) return;
    try {
      await api.delete(`/super-admin/time-zones/${id}`);
      fetchData();
    } catch (error) {
      console.error('Failed to delete time zone', error);
      alert('Failed to delete time zone.');
    }
  };

  const handleCancel = () => {
    setEditingId(null);
    setFormData({ name: '', identifier: '', offset: '', description: '', isActive: true });
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await api.put(`/super-admin/time-zones/${editingId}`, formData);
      } else {
        await api.post('/super-admin/time-zones', formData);
      }
      handleCancel();
      fetchData();
    } catch (error) {
  ...
  const filteredData = data.filter(i =>
    ...
    i.identifier.toLowerCase().includes(searchTerm.toLowerCase()) ||
    i.offset.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  const formContent = (
    <form onSubmit={handleSave} className="flex flex-col h-full gap-4">
      <div className="flex flex-col gap-1.5">
        <label className="text-[12px] font-semibold text-zinc-700">Time Zone Name <span className="text-red-500">*</span></label>
        <input
          type="text"
          required
          value={formData.name}
      ...
      <div className="flex flex-col gap-1.5">
        <label className="text-[12px] font-semibold text-zinc-700">Time Zone Identifier (TZ) <span className="text-red-500">*</span></label>
        <input
          type="text"
          required
      ...
      <div className="flex flex-col gap-1.5">
        <label className="text-[12px] font-semibold text-zinc-700">UTC Offset <span className="text-red-500">*</span></label>
        <select
          required
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <textarea
          ...
          placeholder="Enter description about this time zone"
      ...
      <div className="mt-auto pt-6 flex items-center justify-between gap-3">
        ...
        >
          {editingId ? 'Update Time Zone' : 'Save Time Zone'}
        </button>
      </div>
    </form>
  );
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-zinc-100">
              ...
              <th className="py-1.5 px-3 text-[11.5px] font-bold text-zinc-900 whitespace-nowrap">Time Zone Name</th>
              <th className="py-1.5 px-3 text-[11.5px] font-bold text-zinc-900 whitespace-nowrap">Time Zone Identifier (TZ)</th>
              <th className="py-1.5 px-3 text-[11.5px] font-bold text-zinc-900 whitespace-nowrap">UTC Offset</th>
          ...
          <tbody className="text-[12px] text-zinc-700">
            {loading ? (
              ...
              <tr><td colSpan={9} className="py-8 text-center text-zinc-500">No time zones found.</td></tr>
            ) : (
              paginatedData.map((item, idx) => (
                <tr key={item._id} className="border-b border-zinc-50 hover:bg-zinc-50/50 transition-colors">
                  ...
                  <td className="py-1.5 px-3 whitespace-nowrap">{item.identifier}</td>
                  <td className="py-1.5 px-3 whitespace-nowrap">{item.offset}</td>
  ...
  return (
    <SettingsSplitLayout
      pageTitle="Time Zone Management"
      pageSubtitle="Manage all time zones used across the platform"
      breadcrumbSteps={['Home', 'Settings', 'Time Zone Management']}
      formTitle={editingId ? "Edit Time Zone" : "Add New Time Zone"}
      formSubtitle={editingId ? "Update existing time zone details" : "Create a new time zone configuration"}
      listTitle="Time Zone List"
      listSubtitle="All time zones configured in the system"
