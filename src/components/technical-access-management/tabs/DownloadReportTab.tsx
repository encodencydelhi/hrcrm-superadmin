...
import {
    Download, LayoutGrid, Eye, Settings, FileDown, LogOut, LogIn,
    CalendarDays, Users, ChevronDown, Filter, FileText, Info, ArrowLeft, ArrowRight,
} from "lucide-react";
...
function AccessActivityTable() {
    const columns: TableCardColumn<ActivityLog>[] = [
        ...
        },
        {
            ...
            header: "Details",
            cellClassName: "py-[7px]",
            render: (log) => <p className="text-[9.5px] text-zinc-600 leading-snug">{log.details}</p>,
        },
        {
            key: "ip",
            header: "IP Address",
            cellClassName: "py-[7px]",
            render: (log) => <p className="text-[10.5px] font-bold text-zinc-700">{log.ipAddress}</p>,
        },
        {
            key: "status",
            header: "Status",
    ...
    return (
        <TableCard
            title="Access Activity Report"
            description="Detailed log of all activities performed during the selected period."
            headerActions={
                <>
                ...
                <>
                    ...
                    <div className="flex items-center gap-1">
                        {["|<", "<"].map((b) => (
                            <button
                                key={b}
                                className="h-6 w-6 rounded border border-zinc-200 flex items-center justify-center text-[9px] font-bold text-zinc-400 bg-white hover:bg-zinc-50"
                            >
                                {b}
                            </button>
                        ))}
                        {["1", "2", "3", "...", "6"].map((b) => (
                            <button
                                key={b}
                                className={`h-6 w-6 rounded border flex items-center justify-center text-[10px] font-medium ${
                                    b === "1"
                        ...
                        {[">", ">|"].map((b) => (
                            <button
                                key={b}
                                className="h-6 w-6 rounded border border-zinc-200 flex items-center justify-center text-[9px] font-bold text-zinc-600 bg-white hover:bg-zinc-50"
                            >
                                {b}
                            </button>
                        ))}
...
export default function DownloadReportTab() {
    return (
            ...
            <div className="grid grid-cols-1 xl:grid-cols-4 gap-2 items-start">
                        ...
                        {STATS.map((s, i) => (
                            <div key={i} className="bg-white rounded-lg border border-zinc-200 shadow-sm px-2.5 py-2.5">
                                <div className="flex items-center gap-1.5 mb-1">
                                    <div className={`h-6 w-6 rounded-full ${s.bg} border ${s.border} flex items-center justify-center shrink-0`}>
                                        {s.icon}
                                    </div>
                ...
                <div className="xl:col-span-1 flex flex-col gap-2 w-full">
                    ...
                    <div className="rounded-lg bg-white shadow-sm border border-zinc-200 p-3">
                        ...
                        <p className="text-[9.5px] text-zinc-500 mb-2.5 leading-relaxed">
                            This report includes all activities performed during the technical access period.
                        </p>
                        <div className="space-y-[7px]">
                            ...
                            <div className="flex items-center justify-between gap-2">
                                ...
                                <div className="flex items-center gap-1.5">
                                    <img
                                        src="https://ui-avatars.com/api/?name=Anjali+Sharma&background=random&color=fff&size=20"
                                        alt="Anjali"
